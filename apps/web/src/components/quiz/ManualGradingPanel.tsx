...
import { User, Calendar, FileText, Award, MessageSquare, Loader2, HelpCircle } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Textarea } from '@/components/ui/textarea';
import { Badge } from '@/components/ui/badge';
import { useAttemptDetail, useGradeAttempt, useQuizById } from '@/hooks/api/use-quizzes';
import type { AttemptAnswer, Question } from '@/lib/api-services/quizzes.service';
 ...
 * Асуултын текст + Rubric + Dynamic max оноо + Feedback.
 */
export function ManualGradingPanel({ attemptId, quizId }: ManualGradingPanelProps) {
  ...
  const questionsMap = useMemo<Record<string, Question>>(() => {
    if (!quiz?.questions) return {};
    return Object.fromEntries(quiz.questions.map((q) => [q.questionId, q]));
  }, [quiz]);
    ...
    <div className="space-y-8">
      ...
      {gradableAnswers.length === 0 ? (
        ...
        gradableAnswers.map((answer) => {
          ...
          /** Dynamic quick scores: 0, 50%, 80%, 100% — давхардал арилгах */
          const quickScores = [
            0,
            Math.round(maxPts * 0.5),
            Math.round(maxPts * 0.8),
            maxPts,
              ...
              <div className="p-6 space-y-4">
                ...
                {question?.questionText && (
                  <div className="flex items-start gap-2 p-3 bg-secondary/50 rounded-lg">
                    ...
                    <p className="text-sm font-semibold leading-relaxed">{question.questionText}</p>
                  </div>
                )}
                ...
                {/* Rubric (essay хэрэв байвал) */}
                {question?.rubric?.criteria && question.rubric.criteria.length > 0 && (
                  <div>
                    <h4 className="font-bold text-sm mb-2">{t('gradingTitle')} — Rubric</h4>
                      ...
                      onChange={(e) =>
                        setGradingState((prev) => ({
                          ...
                          [answer.questionId]: {
                            ...
                            points: Math.min(maxPts, Number(e.target.value)),
