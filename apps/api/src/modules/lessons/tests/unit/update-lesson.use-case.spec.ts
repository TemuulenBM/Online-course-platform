...
describe('UpdateLessonUseCase', () => {
  ...
  it('хичээл амжилттай шинэчлэх', async () => {
    const updatedLesson = new LessonEntity({
      ...mockLesson,
      title: 'Шинэчлэгдсэн нэр',
      durationMinutes: 45,
    });
  ...
  it('зөвхөн title шинэчлэх (partial update)', async () => {
    const updatedLesson = new LessonEntity({
      ...mockLesson,
      title: 'Зөвхөн нэр солигдсон',
    });
    lessonRepository.findById.mockResolvedValue(mockLesson);
    lessonRepository.update.mockResolvedValue(updatedLesson);
    lessonCacheService.invalidateAll.mockResolvedValue(undefined);
