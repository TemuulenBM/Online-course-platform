...
export class CertificateProcessor {
  ...
  async handleGenerate(job: Job<{ certificateId: string }>): Promise<void> {
    ...
    try {
      ...
      await this.certificateRepository.update(certificateId, { pdfUrl, qrCodeUrl });
      ...
      // PDF үүсгэлт critical — дахин оролдуулахын тулд exception дахин шидэнэ (Bull retry)
      this.logger.error(
        `Сертификат үүсгэхэд алдаа гарлаа: ${certificateId}`,
  ...
  async handleAutoGenerate(job: Job<{ userId: string; courseId: string }>): Promise<void> {
    ...
    try {
      ...
      // Автомат үүсгэлт critical — Bull retry-г ажиллуулахын тулд дахин шидэнэ
      this.logger.error(
        `Автомат сертификат үүсгэхэд алдаа гарлаа: user=${userId}, course=${courseId}`,
        error instanceof Error ? error.stack : String(error),
      );
      throw error;
    }
