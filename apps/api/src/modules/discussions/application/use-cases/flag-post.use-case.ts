...
/**
 * Нийтлэл тэмдэглэх/тэмдэглэл арилгах (flag toggle) use case.
 * Зөвхөн сургалтын багш эсвэл ADMIN хийх боломжтой.
 */
@Injectable()
export class FlagPostUseCase {
  private readonly logger = new Logger(FlagPostUseCase.name);
  ...
  async execute(
    ...
    if (!isAdmin) {
      ...
      if (course?.instructorId !== userId) {
        throw new ForbiddenException(
          'Зөвхөн сургалтын багш эсвэл админ нийтлэл тэмдэглэх боломжтой',
        );
      }
    }

    /** 3. isFlagged toggle хийх */
    const newFlagged = !post.isFlagged;
    const updated = await this.postRepository.toggleFlag(
      postId,
      newFlagged,
      newFlagged ? flagReason : undefined,
    );
    ...
    this.logger.log(
      `Нийтлэл ${newFlagged ? 'тэмдэглэгдлээ' : 'тэмдэглэл арилгагдлаа'}: ${postId} — хэрэглэгч: ${userId}`,
    );
