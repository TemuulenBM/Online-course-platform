...
export class ProgressCacheService {
  ...
  async getLessonProgress(
    ...
    const cached =
      await this.redisService.get<
        ReturnType<UserProgressEntity['toResponse']>
      >(cacheKey);
    if (cached) {
  ...
  async getCourseProgress(
    ...
    if (cached) {
      this.logger.debug(
        `Кэшнээс сургалтын ахиц олдлоо: ${userId}:${courseId}`,
      );
      return cached.map(
        (item) =>
    ...
    const progressList = await this.progressRepository.findByUserAndCourse(
      userId,
      courseId,
    );
    if (progressList.length > 0) {
      ...
      this.logger.debug(
        `Сургалтын ахиц кэшлэгдлээ: ${userId}:${courseId}`,
      );
    }
  ...
  async invalidateAll(
    ...
    await Promise.all([
      this.redisService.del(
        `${LESSON_PROGRESS_PREFIX}${userId}:${lessonId}`,
      ),
      this.redisService.del(
        `${COURSE_PROGRESS_PREFIX}${userId}:${courseId}`,
      ),
    ]);
    this.logger.debug(
      `Ахицын кэш устгагдлаа: ${userId}:${lessonId}:${courseId}`,
    );
  }
}
